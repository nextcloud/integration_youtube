...
class ConfigController extends Controller {
		...
		try {
			foreach ($values as $key => $value) {
				...
				try {
					...
					return new DataResponse(['error' => $this->l->t('Could not encrypt the Youtube api key')], 500);
				}
			}
			return new DataResponse([]);
		} catch (Exception $e) {
	...
	public function setUserConfig(array $values): DataResponse {
		try {
			...
			return new DataResponse(['error' => $this->l->t('Could not save the user config')], 500);
		}
	}
}
